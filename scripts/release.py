...
class NoReleaseRequired(Exception):
    pass
...
async def _update_changelog_only():
    """Update the Changelog file only, without creating a new release."""

    _setup_psr_release_notes_context([])
    await wait_exec("./psr_env/bin/semantic-release", "--strict", "changelog")

    out, err, exit_code = await communicate("git", "status", "--porcelain")
    out = out.strip()
    if b"CHANGELOG.md" in out and len(out.split(b"\n")) == 1:
        psr_name, psr_email = "semantic-release", "<semantic-release>"
        psr_msg = "ci(changelog): Automatically generated by python-semantic-release"
        env = {
            **os.environ,
            "GIT_AUTHOR_NAME": psr_name,
            "GIT_AUTHOR_EMAIL": psr_email,
            "GIT_COMMITTER_NAME": psr_name,
            "GIT_COMMITTER_EMAIL": psr_email,
        }
        await wait_exec("git", "add", "CHANGELOG.md")
        await wait_exec("git", "commit", "-m", psr_msg, env=env)
        await wait_exec("git", "push", "origin", "main")
    else:
        _logger.warning(
            "Unexpected ‘git status’ output; exit_code=%s\nstdout=%s\nstderr=%s",
            exit_code,
            out,
            err,
        )
...
async def run_subcommand(opts: Opts):
    ...
    except NoReleaseRequired as e:
        if opts.subcommand == "release" and _is_main_branch():
            await _update_changelog_only()
        else:
